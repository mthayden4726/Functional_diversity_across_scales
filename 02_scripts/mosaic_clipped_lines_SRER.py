...
def generate_files():
    for ID in file_ID:
        ...
        dirpath = "SRER_flightlines/Shape_"
        ...
        for file in files:
            ...
            os.remove(flight)
        yield src_files_to_mosaic


for src_files_to_mosaic in generate_files():
    ...
    print(out_meta)
    # Write to computer, send to S
    mosaic_name = Out_Dir + "/mosaic_SRER.tif"
    with rasterio.open(mosaic_name, "w", **out_meta) as dest:
        dest.write(mosaic)
        print("File written")
    # Push to S3 bucket
    destination_s3_key = 'SRER_flightlines/Mosaic_SRER_'+ f"{os.path.basename(src_files_to_mosaic[0].name)}.tif"
...
for i,ID in enumerate(file_ID):
    ...
    # Push to S3 bucket
    destination_s3_key = 'SRER_flightlines/Mosaic_SRER_'+str(i)+'.tif'
    local_file_path = mosaic_name
    upload_to_s3(bucket_name, local_file_path, destination_s3_key)
    print("File uploaded to S3")
